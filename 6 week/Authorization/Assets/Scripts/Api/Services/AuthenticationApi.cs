...
namespace Api.Services
{
    public class AuthenticationApi
    {
        public void SendRegistrationRequest(string username, string password,
            Action<UserProfile> onSuccess, Action<int> onError)
        {
        ...
        public void SendLoginRequestByDeviceId(Action<UserProfile> onSuccess, Action<int> onError)
        {
            var url = Endpoints.API_URL + Endpoints.LOGIN_URL;
            var authenticationRequest = new AuthenticationRequest
            {
            ...
            Action<UserProfile> onSuccess, Action<int> onError)
        {
            ...
            if (webRequest.result != UnityWebRequest.Result.Success)
            {
                ...
                {
                    onError?.Invoke(Convert.ToInt32(webRequest.responseCode));
                    yield break;
                }
